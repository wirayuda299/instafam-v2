  ...
  const getComments = useCallback(async () => {
    try {
      ...
      if (res.length > 0) {
        ...
        setLastCreatedAt(res[res.length - 1].created_at);
      } else {
    ...
    <>
      ...
      {hasMoreComments && prevComments.length >= 10 && (
        <div className="flex w-full animate-pulse gap-3" ref={ref}>
          <div className="bg-black-1 size-14 min-w-14 rounded-full"></div>
          <div className="w-full space-y-3">
            <div className="bg-black-1 h-6 w-full rounded-md"></div>
            <div className="bg-black-1 h-5 w-full max-w-[calc(100%-200px)] rounded-md"></div>
          </div>
        </div>
      )}
    </>
